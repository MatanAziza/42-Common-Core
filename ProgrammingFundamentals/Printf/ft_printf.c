...
int	ft_redirect(const char *flag, va_list list)
{
	if (flag[1] == 'c')
		...
		return (ft_putnbr_base(va_arg(list, int), "0123456789", 1));
	else if (flag[1] == 'i')
		return (ft_putnbr_base(va_arg(list, int), "0123456789", 1));
	else if (flag[1] == 'u')
		return (ft_putnbr_base(va_arg(list, unsigned int), "0123456789", 0));
	else if (flag[1] == 'x')
		...
		return (ft_print_address((unsigned long)va_arg(list, void *),
				"0123456789abcdef"));
}
...
int	ft_printf(const char *flag, ...)
{
	...
	while (*flag)
	{
		...
		{
			count += ft_redirect(flag, list);
			flag += 2;
		}
		else
		{
			count += ft_putchar(flag[0]);
			flag++;
		}
	}
	va_end(list);
